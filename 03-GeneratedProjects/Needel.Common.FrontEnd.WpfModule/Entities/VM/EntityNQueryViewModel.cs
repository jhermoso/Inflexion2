...
namespace Needel.Common.FrontEnd.WpfModule
{
    ...
    /// </summary>
    public partial class EntityNQueryViewModel : Inflexion2.UX.WPF.MVVM.CRUD.QueryViewModel<EntityNViewModel, EntityNView, Int32>
    {
        ...
        public EntityNQueryViewModel(string parentViewName = null)
            :base()
        {
            this.ParentViewName = parentViewName;
            if (!this.IsDesignTime)
            {
                this.ChangeActivateStatusCommand = new DelegateCommand(this.ResetEntityN);
            }
            ...
            var locBinding = new WPFLocalizeExtension.Extensions.LocTextExtension("EntityNPluralEntity");
        ...
        /// <param name="parentViewName"></param>
        public EntityNQueryViewModel( IList<EntityNDto> collection, string parentViewName)
            : this(parentViewName)
        {
            if (collection != null)
            foreach (var item in collection)
            {
                this.Items.Add(new EntityNViewModel(item));
            }
        }
                ...
                try
                {
                    ...
                    serviceClient.BeginGetPaged(this.Specification,                                                 (asyncResult) =>
                    {
                        ...
                        if (result != null)
                        {
                            ...
                            this._items = new ObservableCollection<EntityNViewModel>(result.Select(i => new EntityNViewModel(i/*, first, result.GetPrevious(i), result.GetNext(i), last*/, true)));
        ...
        public void MoveDataFromSrcToDest( EntityNQueryViewModel dataContextTarget, EntityNDto data)
        {
            ...
            dynamic currentView = views.FirstOrDefault(c => c as MNZView != null);
        
            if (this.Title.Equals("AllEntityN"))
            {
                currentView.DataContext.AttachEntityN(dataContextTarget, data);
                //move from "all" means to add new association
                //dataContextTarget.AttachEntityN(data);
            }
            else
            {
                currentView.DataContext.RemoveEntityN(this.Title, dataContextTarget, data);
                //move to "all" means to remove association
                //dataContextTarget.RemoveEntityN(data);
            }
        }
        #endregion
