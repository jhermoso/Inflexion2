...
namespace Needel.Common.FrontEnd.WpfModule
{
    ...
    public partial class ComponentView : UserControl
    {
        ...
        {
            var ownVm = ((ComponentViewModel)this.DataContext);
            if (ownVm != null )
            {
        ...
        private void OnLoadChildrenQueryViewControl(object sender, RoutedEventArgs e)//**1
        {
            ChildrenQueryView.DataContext = ((ComponentViewModel)this.DataContext).ChildrenQueryVM;
            if (ChildrenQueryView.DataContext != null)
            {
                ((ComponentQueryViewModel)ChildrenQueryView.DataContext).RefreshItems();
            }
        }

        private void OnGotFocusChildrenQueryViewControl(object sender, RoutedEventArgs e)//**2
        {
            ComponentViewModel ownVm = null;
            ComponentQueryViewModel vm = null;

            if (sender.GetType() == ChildrenQueryView.GetType())
            {
                ownVm = ((ComponentViewModel)this.DataContext);
                vm = ((ComponentViewModel)this.DataContext).ChildrenQueryVM;
                ownVm.UnregisterCommands();
                vm.RegisterCommands();
                vm.RefreshCommands();
                e.Handled = true;
        ...
        {
            var dc = ((ComponentViewModel)this.DataContext);
