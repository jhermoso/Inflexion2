...
namespace Needel.Common.Application
{
    ...
    /// <summary>
    /// .en mapping class from Entity to dto <see cref="EntityMMapper"/>
    /// .es Clase pública encargada de mapear los datos de una entidad <see cref="EntityMMapper"/>.
    /// </summary>
    public class EntityMMapper : IEntityMMapper
    {
        ...
        #region CONSTRUCTORS
        /// <summary>
        /// .en Get an instance of <see cref="EntityMMapper"/>
        /// .es Inicializa una nueva instancia de la clase <see cref="EntityMMapper"/>.
        /// </summary>
        public EntityMMapper()
        {
        }
        #endregion
        ...
        /// </summary>
        /// <param name="entityEntityM"></param>
        /// <returns></returns>
        public EntityMDto EntityMapping(EntityM entityEntityM)
        {
            if (entityEntityM == null)
            {
                return null;
            }

            return EntityMapping(entityEntityM, true, false);
        }
        ...
        /// </param>
        /// <returns>
        /// .en returns an DTO of type <see cref="EntityMDto"/>
        /// </returns>
        public EntityMDto EntityMapping(EntityM entityEntityM, bool mapParents , bool mapChildren)
        {
            if (entityEntityM == null)
            {
                return null;
            ...
            EntityMDto dtoEntityM = new EntityMDto();
            dtoEntityM.Id = entityEntityM.Id;
            dtoEntityM.Name = entityEntityM.Name;

            if (mapChildren)
            {
                if (entityEntityM.NZs != null)
                {
                    foreach (var item in entityEntityM.NZs)
                    {
                        var childDto = MNZDtoMapper.ValueObjectMapping(item, false, false);
                        if (childDto != null)
                        {
                            dtoEntityM.NZs.Add(childDto);
                        }
                    }
                }
